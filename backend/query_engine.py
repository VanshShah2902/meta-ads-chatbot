...
7. Round monetary values to 2 decimal places, percentages to 2 decimal places. IMPORTANT: In PostgreSQL, ROUND() requires numeric type. Always wrap the ENTIRE expression in a cast: ROUND( (expression)::numeric, 2 ). Example: ROUND( (SUM(spend) / NULLIF(SUM(clicks), 0))::numeric, 2 ). The ::numeric must be OUTSIDE the parentheses wrapping the full expression.
